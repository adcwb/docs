...
C -> S : SYN=1, seq=x (Client发送SYN包，初始序列号为x)
note left: 客户端进入SYN_SENT状态\n发送同步序列编号(SYN)包\n初始化序列号(ISN)为x

S --> C : SYN=1, ACK=1, seq=y, ack=x+1 (Server响应SYN-ACK包)
note right: 服务器进入SYN_RECEIVED状态\n发送SYN+ACK包\n确认客户端的x+1\n自己的ISN为y

C -> S : ACK=1, seq=x+1, ack=y+1 (Client发送ACK确认)
note left: 客户端进入ESTABLISHED状态\n确认服务器的y+1\n连接建立完成
